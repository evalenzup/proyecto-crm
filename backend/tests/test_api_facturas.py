...
def test_crear_listar_y_folios_consecutivos(client, db_session):
    ...
    emp1 = Empresa(nombre="EMPRESA UNO", nombre_comercial="EMPRESA UNO", ruc="RUC-001", rfc="AAA010101AAA", regimen_fiscal="601", codigo_postal="01000", contrasena="x")
    emp2 = Empresa(nombre="EMPRESA DOS", nombre_comercial="EMPRESA DOS", ruc="RUC-002", rfc="AAA010101AAA", regimen_fiscal="601", codigo_postal="01000", contrasena="x")
    cli1 = Cliente(nombre_comercial="CLIENTE 1", nombre_razon_social="CLIENTE 1 SA DE CV", rfc="XAXX010101000", regimen_fiscal="612", codigo_postal="02020")
    cli2 = Cliente(nombre_comercial="CLIENTE 2", nombre_razon_social="CLIENTE 2 SA DE CV", rfc="XAXX010101000", regimen_fiscal="612", codigo_postal="02020")
    ...
    r = client.post("/api/factura/", json=construir_factura_payload(str(emp1.id), str(cli1.id)))
    assert r.status_code == 201
    f2 = r.json()
    assert f2["folio"] == 2

    # Primera factura de emp2 → folio 1 (consecutivo por empresa)
    r = client.post("/api/factura/", json=construir_factura_payload(str(emp2.id), str(cli2.id)))
    assert r.status_code == 201
    f3 = r.json()
    assert f3["folio"] == 1
...
def test_timbrar_cancelar_y_pago(client, db_session):
    emp = Empresa(nombre="EMPRESA TIMBRE", nombre_comercial="EMPRESA TIMBRE", ruc="RUC-T", rfc="AAA010101AAA", regimen_fiscal="601", codigo_postal="01000", contrasena="x")
    cli = Cliente(nombre_comercial="CLIENTE T", nombre_razon_social="CLIENTE T SA DE CV", rfc="XAXX010101000", regimen_fiscal="612", codigo_postal="02020")
    ...
    r = client.post("/api/factura/", json=construir_factura_payload(str(emp.id), str(cli.id)))
    assert r.status_code == 201
    fac = r.json()
    ...
    r = client.patch(f"/api/factura/{fac['id']}/pago", params={"status": "PAGADA", "fecha_cobro": date.today().isoformat()})
    assert r.status_code == 200
    fac_p = r.json()
