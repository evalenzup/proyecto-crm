...
def _draw_header(c: canvas.Canvas, f: Factura, logo_path: Optional[str]) -> float:
    ...
    x_space = 70
    c.setFont(FONT_B, 8)
    c.drawString(CONTENT_X0, y_left, "Fecha emisión:")
    c.setFont(FONT, 8)
    c.drawString(
        CONTENT_X0 + x_space,
        y_left,
    ...
    if f.serie:
        c.setFont(FONT_B, 8)
        c.drawString(CONTENT_X0, y_left, "Serie y Folio:")
        c.setFont(FONT, 8)
        c.drawString(CONTENT_X0 + x_space, y_left, f"{f.serie} {f.folio}")
        y_left -= 12
    else:
        c.setFont(FONT_B, 8)
        c.drawString(CONTENT_X0, y_left, "Folio:")
        c.setFont(FONT, 8)
        c.drawString(CONTENT_X0 + x_space, y_left, str(f.folio))
        y_left -= 12
    if f.cfdi_uuid:
        c.setFont(FONT_B, 8)
        c.drawString(CONTENT_X0, y_left, "UUID:")
        c.setFont(FONT, 8)
        c.drawString(CONTENT_X0 + x_space, y_left, f.cfdi_uuid)
        y_left -= 12
...
def _draw_kv_block(
    ...
    y = _draw_paragraph_box(
        c, value, x, y, max_w, font=value_font, size=value_size, leading=value_leading
    )
    return y - gap
...
def _draw_footer(c: canvas.Canvas, f: Factura, is_last_page: bool):
    ...
    if uso_cfdi_lbl:
        ...
        c.setFont(FONT, 8)
        c.drawString(CONTENT_X0 + 90, base_y, uso_cfdi_lbl)
        base_y -= 12
    if metodo_lbl:
        c.setFont(FONT_B, 8)
        c.drawString(CONTENT_X0, base_y, "Método de pago:")
        c.setFont(FONT, 8)
        c.drawString(CONTENT_X0 + 90, base_y, metodo_lbl)
        base_y -= 12
    if forma_lbl:
        c.setFont(FONT_B, 8)
        c.drawString(CONTENT_X0, base_y, "Forma de pago:")
        c.setFont(FONT, 8)
        c.drawString(CONTENT_X0 + 90, base_y, forma_lbl)
        base_y -= 12
    if moneda_lbl:
        c.setFont(FONT_B, 8)
        c.drawString(CONTENT_X0, base_y, "Moneda:")
        c.setFont(FONT, 8)
        c.drawString(CONTENT_X0 + 90, base_y, moneda_lbl)
        base_y -= 12
    if tipo_cambio_lbl and moneda_lbl and moneda_lbl != "Peso Mexicano":
        c.setFont(FONT_B, 8)
        c.drawString(CONTENT_X0, base_y, "Tipo de cambio:")
        c.setFont(FONT, 8)
    ...
    c.setFont(FONT_B, 8)
    c.drawRightString(right_x, totales_y, f"Subtotal: {_money(f.subtotal)}")
    totales_y -= espacio_y
    ...
    if tras_list:
        for (imp_name, tasa), monto in tras_list:
            c.drawRightString(
                right_x, totales_y, f"{imp_name} {_pct(tasa, 2)}: {_money(monto)}"
            )
            totales_y -= espacio_y
        ...
        c.drawRightString(
            right_x,
            totales_y,
            f"Impuestos trasladados: {_money(f.impuestos_trasladados)}",
        )
        totales_y -= espacio_y
    ...
    if ret_list:
        for (imp_name, tasa), monto in ret_list:
            c.drawRightString(
                right_x,
                totales_y,
                f"Retención {imp_name} {_pct(tasa, 4)}: {_money(monto)}",
            )
            totales_y -= espacio_y
    elif (f.impuestos_retenidos or 0) > 0:
        c.drawRightString(
            right_x, totales_y, f"Impuestos retenidos: {_money(f.impuestos_retenidos)}"
        )
        totales_y -= espacio_y
    ...
    def draw_block(label, value, y, font_label=FONT_B, font_value=FONT, size=6):
        if not value:
            return y
        c.setFont(font_label, size)
        c.drawString(CONTENT_X0, y, label)
        y -= 10
        lines = _wrap_lines(c, value, max_w, font=font_value, size=size)
        c.setFont(font_value, size)
        for ln in lines:
            c.drawString(CONTENT_X0, y, ln)
            y -= 9
        return y - 4
...
def _concept_row(cpt: FacturaDetalle) -> List[Paragraph | str]:
    ...
    unidad_desc_hint = getattr(cpt, "unidad_descripcion", None) or getattr(
        cpt, "clave_unidad_desc", None
    )
    ...
    return [
        ...
        Paragraph(
            str(getattr(cpt, "descripcion", "") or ""), p_left_6
        ),  # 1 Descripción
