...
def _fmt_cfdi_fecha_local(dt: Optional[datetime]) -> str:
    if dt is None:
        dt = datetime.now(TIJUANA_TZ)
    elif dt.tzinfo is not None:
        dt = dt.astimezone(TIJUANA_TZ)
...
def _load_csd_cert_for_empresa(empresa) -> Tuple[Optional[str], Optional[str], Optional[bytes]]:
    try:
...
def _load_csd_key_for_empresa(empresa):
    ...
    if not os.path.exists(key_path):
        if logger: logger.info(f"[CSD] No existe KEY: {key_path}")
        else: print(f"[CSD] No existe KEY: {key_path}")
        return None

    pwd = _get_empresa_csd_password(empresa)
    if not pwd:
        if logger: logger.warning("[CSD] Password DB vacía/nula")
        else: print("[CSD] Password DB vacía/nula")
        return None
    ...
    if logger: logger.warning("[CSD] No se pudo abrir KEY (DER/PEM/P12).")
    else: print("[CSD] No se pudo abrir KEY (DER/PEM/P12).")
    return None
...
def _find_xslt_cadena40_path() -> Optional[str]:
    ...
    candidates.append(os.path.join(base, "sat", "cadenaoriginal_4_0", "cadenaoriginal_4_0.xslt"))

    if logger: logger.info("Buscando XSLT en settings/candidates:")
    else: print("Buscando XSLT en settings/candidates:")
    for c in candidates:
        try:
            ...
            (logger.info if logger else print)(f"  - {c} -> {'OK' if exists else 'NO'} {info}")
        except Exception as e:
...
def _apply_xslt20_with_saxon(xml_bytes: bytes, xslt_path: str) -> Optional[str]:
    ...
    try:
        ...
        try: os.remove(xslt_local)
        except Exception: pass

def _build_cadena_original_40(xml_bytes: bytes) -> Optional[str]:
    xslt_path = _find_xslt_cadena40_path()
    if not xslt_path:
        (logger.error if logger else print)("No se encontró el XSLT de cadena original.")
        return None
...
def _read_pago_attrs_from_factura(f: Factura) -> tuple[str, Optional[str], Optional[str]]:
    """
...
def _apply_attrs_pago_cfdi40_strict(attrs: dict, *, tipo: str, metodo_pago: Optional[str], forma_pago: Optional[str]):
    """
    Reglas sin defaults:
      - I/E:
    ...
    if t in {"I", "E"}:
        if not mp:
            raise RuntimeError("MetodoPago es requerido para TipoDeComprobante I/E y viene vacío en la factura (BD).")
        attrs["MetodoPago"] = mp
        ...
        if mp == "PPD":
            ...
            if not fp:
                raise RuntimeError("FormaPago es requerida cuando MetodoPago != 'PPD' (I/E) y viene vacía en la factura (BD).")
...
def _calc_totales_desde_conceptos(conceptos: List[FacturaDetalle]) -> Dict[str, Decimal]:
    subtotal = Decimal("0"); descuento = Decimal("0"); base_gravada = Decimal("0")
    for c in conceptos:
        ...
        subtotal += imp_sin_iva; descuento += dsc; base_gravada += (imp_sin_iva - dsc)
    return dict(subtotal=subtotal, descuento=descuento, base_gravada=base_gravada)

def _group_traslados_por_tasa(bases_e_tasas: List[Tuple[Decimal, Decimal]]) -> Tuple[Decimal, List[Tuple[str,str,str,Decimal,Decimal]]]:
    grupos: Dict[str, Dict[str, Decimal]] = {}; total = Decimal("0")
    for base, tasa in bases_e_tasas:
        if base <= 0 or tasa <= 0: continue
        imp = (base * tasa).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        total += imp
        key = tasa6(tasa)
        if key not in grupos: grupos[key] = {"base": Decimal("0"), "importe": Decimal("0")}
        grupos[key]["base"] += base; grupos[key]["importe"] += imp
...
def _tipo_cert(subject: "x509.Name", cert_obj: "x509.Certificate | None" = None) -> str:
    ...
    if "FIEL" in s or "E.FIRMA" in s or "FIRMA ELECTR" in s: return "FIEL"
    if "SELLO" in s: return "CSD"
    try:
...
def build_cfdi40_xml_sin_timbrar(db: Session, factura_id: UUID) -> bytes:
    ...
    base_gravada = calc["base_gravada"]
    tasa_global  = (traslados / base_gravada) if (traslados > 0 and base_gravada > 0) else D(0)
    total = D(getattr(f, "total", None) or (subtotal - descuento + traslados - retenidos))
    ...
    compro_attrs = {
        ...
        **({"TipoCambio": qty_any(f.tipo_cambio, 6)} if getattr(f, "tipo_cambio", None) else {}),
        "Total": money2(total),
        "TipoDeComprobante": (getattr(f, "tipo_comprobante", None) or "I").upper(),
        "Exportacion": getattr(f, "exportacion", None) or "01",
        "LugarExpedicion": (getattr(f, "lugar_expedicion", None) or (getattr(emp, "codigo_postal", None) or ""))[:5],
    ...
    _apply_attrs_pago_cfdi40_strict(compro_attrs, tipo=tipo_db, metodo_pago=mp_db, forma_pago=fp_db)
    ...
    if no_cert:   compro.set("NoCertificado", no_cert)
    if cert_b64:  compro.set("Certificado", cert_b64)

    if _CRYPTO_OK and cert_der:
        cert_obj = x509.load_der_x509_certificate(cert_der)
        tipo = _tipo_cert(cert_obj.subject, cert_obj)
        subj = cert_obj.subject.rfc4514_string()
        (logger.info if logger else print)(f"[CSD check] subject={subj} tipo_inferido={tipo}")
        if tipo == "FIEL":
            raise RuntimeError("Se detectó un certificado de FIEL/e.firma. Debes usar el CSD.")

    # CfdiRelacionados
    if getattr(f, "cfdi_relacionados_tipo", None) and getattr(f, "cfdi_relacionados", None):
        rel = SubElement(compro, f"{{{NS_CFDI}}}CfdiRelacionados", {"TipoRelacion": f.cfdi_relacionados_tipo})
    ...
    # Emisor
    SubElement(compro, f"{{{NS_CFDI}}}Emisor", {
        "Rfc": getattr(emp, "rfc", "") or "",
        **({"Nombre": getattr(emp, "nombre", None)} if getattr(emp, "nombre", None) else {}),
    ...
    if _is_global_invoice(f):
        receptor_attrs = {
            ...
            "DomicilioFiscalReceptor": (getattr(f, "lugar_expedicion", None) or (getattr(emp, "codigo_postal", None) or ""))[:5],
        ...
        receptor_attrs = {
            ...
            **({"Nombre": cli_nombre} if cli_nombre else {}),
            "DomicilioFiscalReceptor": (getattr(cli, "codigo_postal", None) or (getattr(f, "lugar_expedicion", None) or ""))[:5],
    ...
    if per and mes and anio:
        SubElement(compro, f"{{{NS_CFDI}}}InformacionGlobal", {
    ...
    for cpt in conceptos:
        ...
        base     = (imp - dsc)
        objeto = "02" if (base > 0 and getattr(cpt, "iva_tasa", None) not in (None, Decimal("0"))) else "01"
        c_attrs = {
            "ClaveProdServ": getattr(cpt, "clave_producto", None) or "01010101",
            **({"NoIdentificacion": str(getattr(cpt, "no_identificacion"))} if getattr(cpt, "no_identificacion", None) else {}),
        ...
        if objeto == "02" and base > 0:
            ...
            SubElement(traslados_c, f"{{{NS_CFDI}}}Traslado", {
                "Base": qty_any(base, 6), "Impuesto": "002", "TipoFactor": "Tasa",
                "TasaOCuota": tasa6(tasa_usada),
                "Importe": qty_any((base * tasa_usada).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP), 6),
            })
    ...
    if total_tras > 0:
        impuestos_node = SubElement(compro, f"{{{NS_CFDI}}}Impuestos", {
        ...
        for imp_clave, tipo_factor, tasa, base_sum, imp_sum in filas:
            SubElement(traslados_res, f"{{{NS_CFDI}}}Traslado", {
                "Base": money2(base_sum), "Impuesto": imp_clave, "TipoFactor": tipo_factor,
                "TasaOCuota": tasa, "Importe": money2(imp_sum),
            })
    ...
    if not cadena:
        raise RuntimeError("No se pudo generar la Cadena Original 4.0. Revisa los logs anteriores (ruta, versión XSLT y resolver).")

    private_key = _load_csd_key_for_empresa(emp)
    if not private_key:
        raise RuntimeError("No se pudo cargar la llave privada del CSD (verifica .key y contraseña en DB).")
