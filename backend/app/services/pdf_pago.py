...
def _draw_pago_header(c: canvas.Canvas, p: Pago, logo_path: Optional[str]) -> float:
    ...
    if p.serie or p.folio:
        ...
        c.setFont(FONT, 8)
        c.drawString(CONTENT_X0 + x_space, y_left, f"{p.serie or ''} {p.folio}")
        y_left -= 12
    if p.uuid:
        c.setFont(FONT_B, 8)
        c.drawString(CONTENT_X0, y_left, "Folio Fiscal (UUID):")
        c.setFont(FONT, 8)
        c.drawString(CONTENT_X0 + x_space, y_left, p.uuid)
        y_left -= 12
    if p.fecha_timbrado:
        c.setFont(FONT_B, 8)
    ...
    if p.empresa and p.empresa.codigo_postal:
        ...
        c.setFont(FONT, 8)
        c.drawString(CONTENT_X0 + x_space, y_left, p.empresa.codigo_postal)
        y_left -= 12
    ...
    if p.empresa:
        y_left = _draw_label_wrap(
            c, "Nombre", p.empresa.nombre, CONTENT_X0, y_left, left_w
        )
        y_left = _draw_label_wrap(c, "RFC", p.empresa.rfc, CONTENT_X0, y_left, left_w)
        y_left = _draw_label_wrap(
            c,
            "Régimen",
            _regimen_label(p.empresa.regimen_fiscal),
            CONTENT_X0,
            y_left,
            left_w,
        )
    ...
    if p.cliente:
        y_right = _draw_label_wrap(
            c, "Nombre", p.cliente.nombre_razon_social, x_right, y_right, right_w
        )
        y_right = _draw_label_wrap(c, "RFC", p.cliente.rfc, x_right, y_right, right_w)
        y_right = _draw_label_wrap(
            c,
            "Régimen",
            _regimen_label(p.cliente.regimen_fiscal),
            x_right,
            y_right,
            right_w,
        )
        y_right = _draw_label_wrap(
            c, "Uso CFDI", "CP01 - Pagos", x_right, y_right, right_w
        )
...
def _draw_pago_footer(c: canvas.Canvas, p: Pago):
    ...
    def draw_block(label, value, y_start):
        if not value:
            return y_start
        c.setFont(FONT_B, 6)
        c.drawString(qr_right_x, y_start, label)
        y = y_start - 8
        lines = _wrap_lines(c, value, max_w, font=FONT, size=5)
        c.setFont(FONT, 5)
        for ln in lines:
            c.drawString(qr_right_x, y, ln)
            y -= 7
        return y - 4
    ...
    if p.no_certificado:
        ...
        c.drawString(
            qr_right_x,
            y,
            f"No. de Serie del Certificado del Emisor: {p.no_certificado}",
        )
        y -= 10
    if p.no_certificado_sat:
        ...
        c.drawString(
            qr_right_x,
            y,
            f"No. de Serie del Certificado del SAT: {p.no_certificado_sat}",
        )
        y -= 10
