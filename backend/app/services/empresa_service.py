...
class EmpresaRepository(BaseRepository[Empresa, EmpresaCreate, EmpresaUpdate]):
    ...
    def create(
        ...
        *,
        ...
        resultado = CertificadoService.validar_bytes(cer_bytes, key_bytes, obj_in.contrasena)
        if not resultado["valido"]:
            logger.info("❌ EmpresaRepository.create: validación falló → %s", resultado["error"])
    ...
    def update(
        ...
        *,
        ...
        cer_abs = join(settings.CERT_DIR, basename(db_obj.archivo_cer)) if db_obj.archivo_cer else None
        key_abs = join(settings.CERT_DIR, basename(db_obj.archivo_key)) if db_obj.archivo_key else None
        ...
        if cer_new and key_new:
            ...
            resultado = CertificadoService.validar_bytes(cer_bytes, key_bytes, obj_in.contrasena)
            if not resultado["valido"]:
                logger.info("❌ EmpresaRepository.update: validación falló → %s", resultado["error"])
                raise HTTPException(status_code=400, detail=resultado["error"] or "Certificado inválido")
            if resultado.get("valido_hasta") and datetime.fromisoformat(resultado["valido_hasta"]).date() < date.today():
                raise HTTPException(status_code=400, detail="El certificado está vencido.")
            ...
            logger.info("✅ EmpresaRepository.update: certificados reemplazados y contraseña actualizada")
        else:
            ...
            if "contrasena" in update_data and update_data["contrasena"]:
                ...
                logger.info("ℹ️ EmpresaRepository.update: contraseña actualizada (sin cambiar certificados)")

        # Logo
        if logo:
            try:
                if db_obj.logo and os.path.exists(os.path.join(settings.DATA_DIR, db_obj.logo)):
    ...
    def remove(self, db: Session, *, id: UUID) -> Optional[Empresa]:
        ...
        for fname in (empresa.archivo_cer, empresa.archivo_key):
            if fname:
                ...
                try:
                    if os.path.exists(path): os.remove(path) # type: ignore
                except Exception as e:
                    logger.error(f"Error al eliminar archivo de certificado {path}: {e}")
        
        if empresa.logo:
            logo_path = os.path.join(settings.DATA_DIR, empresa.logo) # type: ignore
            try:
                if os.path.exists(logo_path): os.remove(logo_path) # type: ignore
            except Exception as e:
