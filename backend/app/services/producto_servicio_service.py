...
class ProductoServicioRepository(BaseRepository[ProductoServicio, ProductoServicioCreate, ProductoServicioUpdate]):
    ...
    def create(self, db: Session, *, obj_in: ProductoServicioCreate) -> ProductoServicio:
        ...
        self._validar_descripcion_unica(db, data["descripcion"], data["empresa_id"])

        if data["tipo"] == "PRODUCTO":
            self._validar_datos_producto(data)
        else:
            self._limpiar_campos_inventario(data)
    ...
    def update(self, db: Session, *, db_obj: ProductoServicio, obj_in: ProductoServicioUpdate) -> Optional[ProductoServicio]:
        ...
        if "clave_producto" in update_data or "clave_unidad" in update_data:
            ...
            clave_uni = update_data.get("clave_unidad", db_obj.clave_unidad)
            self._validar_campos_sat(clave_prod, clave_uni)

        tipo = update_data.get("tipo", db_obj.tipo)
        if tipo == "PRODUCTO":
            self._validar_datos_producto(update_data)
        else:
            self._limpiar_campos_inventario(update_data)
